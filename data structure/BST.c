...
int count_leaf(TNode* n){  //단말노드의 개수
    ...
    return (n->left == NULL && n->right == NULL) ? 1 : count_leaf(n->left) + count_leaf(n->right); //왼쪽,오른쪽 자식 둘다 없으면 +1, 하나라도 있으면 내려감.
}
...
void delete(TNode *parent, TNode *node){  //삭제연산, 삭제할 노드와 그 부모 노드를 받음
    ...
    if(node->left == NULL && node -> right == NULL) {  //단말노드 삭제
        ...
        if(node == root) {
            root = child;  //삭제할 노드가 루트였다면 자식노드가 루트가 됨
        } else {
            if(parent->left == node) {
                parent->left = child;
            }
